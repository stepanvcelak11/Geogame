...
- **Tři pracoviště**, každé s vlastním S-JTSK: **Stavba RD, Nová Ves** (ulice s vpustmi, trafostanice s nivelační značkou, parcela 1254/3), **Louka u Kněžívky** (polní cesta, meze se stromořadím, pozemek 812/5, TB na kopci) a **Lesní cesta u Hrušova** (louka s bodem 6101 a TB, za ní hustý smrkový les s cestou, propustkem, hraničním kamenem a památným dubem).
- Zakázka **Zaměření propustku na lesní cestě** (les): v lese GNSS nedá ani FLOAT a bodové pole tam není. Na cestě před lesem si roverem stabilizuješ dva **pomocné body** 8001 a 8002 (hřeb, 30 s observace, jen s FIXem – jejich souřadnice nesou chybu GNSS), stanici postavíš na 8001, orientuješ na 8002 a do lesa zaměříš propustek (kód Propustek), hraniční kámen a patu dubu.
...
- **Měření ve dvou polohách dalekohledu** (přepínač I+II): průměr ruší kolimační (c / sin z) a indexovou chybu stanice po pádu; rozdíl 2c v cc ukáže, že přístroj potřebuje rektifikaci.
- Zakázka **Studánka v průseku (polygonový pořad)** (les): studánka je hluboko v lese, z louky na ni není vidět. Stanice na 8001 (orientace 8002) výtyčkou s hranolem stabilizuje u ústí průseku **bod pořadu 8101** (změří ho a zatluče se hřeb), stanice se na něj přestaví, orientuje zpět na 8001 a zaměří studánku a hraniční kámen v průseku. Chyby pomocných bodů a orientace se po pořadu přenášejí dál.
- Zakázka **Vodovodní přípojka před zásypem** (stavba): otevřený výkop s modrou trubkou PE, zaměřit konec u domu, dva lomy a napojení na řad (kód Vodovod). Bagr výkop zasype **dvě herní hodiny po převzetí** – „Co dál?“ ukazuje, kolik zbývá; po zásypu už potrubí zaměřit nejde a objednatel reklamuje.
- Zakázka **Geometrický plán: kůlna na louce**: GNSS rohy kůlny a **oměrné míry pásmem** – s prázdnýma rukama zamiř na roh, „Pásmo: přiložit“ (Pepa drží nulu), u sousedního rohu „odečíst“. V kanceláři se míry porovnají s délkami ze souřadnic (do 8 cm) a objednatel čeká výstup Geometrický plán.
- Kontrola na bodu bodového pole změřená jen s FLOAT se hned ohlásí a nepočítá se – při odevzdání už se nesvádí na chybu nastavení kontroleru.
- **Stavba dálnice D35 – 3D řízení finišeru**: nová lokalita (rozestavěný pravý pás s podkladem, hotový levý pás se značením, protihluková stěna a sloupy osvětlení se štítky, staveništní cesta; v krajině odbočka „D35 – stavba dálnice“). Zakázka **3D řízení finišeru a kontrola pokládky**: volné stanovisko na štítky → program stanice → řídicí panel 3D ve finišeru (výběr modelu vrstvy, kalibrace desky, připojení stanice, start) → finišer pokládá ložnou vrstvu ACL 22+ (roste nový asfalt, za ním jezdí válec). Chyba výšky stanoviska, špatný model (+4 cm) nebo nekalibrovaná deska (+18 mm) se položí do asfaltu; nad 150 m od stanice je potřeba ji přestavit dopředu, při ztrátě zámku finišer zastaví (příčná spára). Nakonec **kontrola za finišerem**: 3 profily × vlevo/osa/vpravo na sprejových značkách, tolerance ±10 mm, výstup Protokol kontroly pokládky.
- **Odrazné štítky**: na trafostanici, stavební buňce a sloupu vedení jsou štítky 901–903 (známé Y, X, H). Stanice je měří bez hranolu (zamířit přesně na terč), orientace i volné stanovisko na ně fungují; když na štítek míříš v režimu hranol, stanice poradí přepnout. Nová zakázka **Volné stanovisko na štítky** (obtížnost 2).
- **Jízda po krajině**: do lokalit můžeš dojet sám. „Jet sám“ v tabletu načte krajinu 1,5 × 1,5 km se silniční sítí (křižovatka s vesničkou, pole, lesy, sloupy vedení, směrovky a cedule obcí); dodávka vyjede z výjezdu lokality a na konci silnice k cíli se načte cílová lokalita. Cesta trvá skutečný herní čas, šipka a mapa v tabletu ukazují směr. „Rychlý přesun“ zůstává.
- **Program totální stanice** (po vzoru Leica Captivate): zakázka, import bodového pole, typ hranolu s adiční konstantou (GPR1 0,0 / mini +17,5 / 360° +23,1 / fólie +34,4 mm), výška cíle, atmosférická korekce (teplota, tlak → ppm) a přehled stanoviska. Otevře se sám po ustavení (bez zakázky a importu stanice neví, kde stojí), jinak tlačítkem Program v dalekohledu. Stanice po předchozím uživateli má nastavený 360° hranol a výšku cíle 1,5 m – kdo to nepřenastaví, dostane kontrolní délku +23 mm a reklamaci.
- Zakázka **Zaškolení: první měření totální stanicí** (obtížnost 1, stavba): celý postup se stanicí krok za krokem už od začátku kariéry.
- **Pepa (pomocník)**: nová postava s klouby (kolena, lokty, krk), montérky, reflexní vesta, helma na stavbě / kšiltovka jinde a knír; animace chůze, dýchání, držení výtyčky, práce u přístroje, kufr v ruce a stativ na rameni. Umí navíc: **přinést věc z auta**, **odnést věc do auta**, **přijet s dodávkou** k tobě, **poradit, co dál a proč**, a jedním povelem **postavit hranol na orientační bod** stanice.
